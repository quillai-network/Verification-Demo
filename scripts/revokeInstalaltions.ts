...
async function main() {
  ...
  const requiredVars = ["CLIENT_AGENT_PRIVATE_KEY", "SERVER_AGENT_PRIVATE_KEY", "XMTP_DB_ENCRYPTION_KEY", "XMTP_ENV"];
  ...
  try {
    ...
    const user = createUser(envVars.SERVER_AGENT_PRIVATE_KEY as `0x${string}`);
