...
import { Wallet, HDNodeWallet, keccak256, toUtf8Bytes, verifyMessage, verifyTypedData, TypedDataDomain } from "ethers";
import { ulid } from "ulid";
import { addrFromCaip10, caip10, deepClone } from "./utils";
import type {
  Bytes32,
  Hex,
  MandateInit,
...
export class Mandate {
  ...
  async sign(
    ...
    if (alg === "eip191") {
      ...
      signature = (await wallet.signTypedData(domain, types as any, value)) as Hex;
    } else {
  ...
  verifyRole(role: "client" | "server", domain?: TypedDataDomain): VerifyResult {
    ...
    if (alg === "eip191") {
      ...
      const types = { Mandate: [{ name: "mandateHash", type: "bytes32" }] as const };
      const value = { mandateHash };
      recovered = verifyTypedData(domain, types as any, value, signature);
    } else {
      throw new Error(`Unsupported alg: ${alg}`);
    }

    // 3) compare against CAIP-10 role address
    const expected = addrFromCaip10((this.m as any)[role]).toLowerCase();
